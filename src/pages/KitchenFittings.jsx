...
import { getKitchenFittingsProductsApi, getAllProductsApi } from '../services/allAPI'
...
function KitchenFittings() {
  ...
  // Fetch only kitchen fittings products
  const getAllKitchen = async () => {
    const result = await getKitchenFittingsProductsApi()
    if (result.status === 200) {
      setAllKitchen(result.data)
    }
  }
  ...
  const getRandomProducts = async () => {
    ...
    if (result.status === 200) {
      const nonKitchen = result.data.filter(item => item.categoryGroup !== "Kitchen Fittings")
      const shuffled = [...nonKitchen].sort(() => 0.5 - Math.random())
      setRandomProducts(shuffled.slice(0, 6))
    }
  }

  // Categories for Kitchen Fittings
  const categories = [
    "All",
    "Kitchen Baskets",
    "Kitchenware",
  ...
  // Brands
  const brands = ["Europa", "Quba", "Boss", "Loctec"]

  // Filtering logic
  const filteredProducts = allKitchen.filter((item) => {
      ...
      <div>
        ...
        <div className='w-100' style={{ backgroundColor: "#e9e9e9ff" }}>
          <div className='container d-flex justify-content-between pt-3 pb-3 '>
            <div className='fs-5'>Kitchen Fittings</div>
            <div>
              <span className='me-3'>
              ...
              /
              <span className='ms-3'>
                <Link className='text-decoration-none text-hover-darkblue' to={'/kitchenfittings'}>Kitchen Fittings</Link>
              </span>
            </div>
          </div>
        </div>
              ...
              <div className="d-flex flex-wrap">
                {categories.slice(0, 3).map((cat, index) => (
                  <Button
                    ...
                    className={`custom-category-btn mt-2 me-2 ${selectedCategory === cat ? "text-white fw-bold" : ""}`}
                ...
                {showAll && categories.slice(3).map((cat, index) => (
                  <Button
                    key={index + 3}
                    variant={selectedCategory === cat ? "primary" : "light"}
                    className={`custom-category-btn mt-2 me-2 ${selectedCategory === cat ? "text-white fw-bold" : ""}`}
              ...
              <Dropdown className="w-100">
                ...
                <Dropdown.Menu className="w-100">
                  {brands.map((brand, idx) => (
                    <Dropdown.Item key={idx} onClick={() => setSelectedBrand(brand)}>
                      {brand}
                    </Dropdown.Item>
                  ))}
                </Dropdown.Menu>
        ...
        <div className='container mt-5'>
          <div className='row g-3'>
            {filteredProducts.length > 0 ? (
              filteredProducts.map((kitchen, idx) => (
                <div key={idx} className='col-12 col-sm-6 col-md-4 col-lg-3 mt-4'>
                  <ProductCard displayData={kitchen} />
                </div>
              ))
            ) : (
        ...
        <div className='ms-4'>
          ...
          <div className="d-flex gap-1 overflow-auto pb-3 px-2" style={{ scrollSnapType: "x mandatory" }}>
        ...
        <div className='container'>
          ...
          <div className="d-none d-md-block">
            <div>
              <h6 className='fw-bold d-inline'><Link className="text-decoration-none text-black" to={'/plywood'}>Plywood</Link>:</h6>
            ...
            <div>
              <h6 className='fw-bold d-inline'><Link className="text-decoration-none text-black" to={'/hardware'}>Hardware</Link>:</h6>
              <span className='ms-2 text-muted border-end pe-2'>Mortise Locks</span>
              <span className='ms-1 text-muted border-end pe-2'>Hinges</span>
              <span className='ms-1 text-muted'>Handles</span>
            </div>
            <div>
              <h6 className='fw-bold d-inline'><Link className="text-decoration-none text-black" to={'/doors'}>Doors</Link>:</h6>
              <span className='ms-2 text-muted border-end pe-2'>Doors</span>
              <span className='ms-1 text-muted'>Door Fittings</span>
            </div>
            <div>
              <h6 className='fw-bold d-inline'><Link className="text-decoration-none text-black" to={'/kitchenfittings'}>Kitchen Fittings</Link>:</h6>
              <span className='ms-2 text-muted border-end pe-2'>Kitchen Baskets</span>
              <span className='ms-1 text-muted'>Kitchenware</span>
            </div>
            <div>
              <h6 className='fw-bold d-inline'><Link className="text-decoration-none text-black" to={'/glass'}>Glass</Link>:</h6>
          ...
          <div className="d-block d-md-none">
            ...
            <div className="mb-3">
              ...
              <div className="d-flex flex-wrap gap-2">
                <span className='badge bg-light text-muted'>Mortise Locks</span>
        ...
        <div className="mt-5">
          <div className="p-5 bg-light rounded shadow-sm text-center bulk-section">
            <h3 className="fw-bold mb-3" style={{ color: "#003366", fontSize: "1.8rem" }}>
              Looking for Bulk Orders?
            </h3>
            <p className="text-muted mb-4 mx-auto" style={{ maxWidth: "600px", fontSize: "1rem" }}>
