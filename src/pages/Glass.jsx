...
import { getGlassProductsApi, getAllProductsApi } from '../services/allAPI'

function Glass() {
  const [showAll, setShowAll] = useState(false)
  const [allGlass, setAllGlass] = useState([])
  ...
  // Fetch only Glass products
  const getAllGlass = async () => {
    const result = await getGlassProductsApi()
    if (result.status === 200) {
      setAllGlass(result.data)
    }
  }
  ...
  const getRandomProducts = async () => {
    ...
    if (result.status === 200) {
      const nonGlass = result.data.filter(item => item.categoryGroup !== "Glass")
      const shuffled = [...nonGlass].sort(() => 0.5 - Math.random())
      setRandomProducts(shuffled.slice(0, 6))
    }
  }

  // Categories
  const categories = ["All", "Glass", "Design Work Glass"]

  // Brands
  const brands = ["Europa", "Quba", "Boss", "Loctec"]

  // Filtering logic
  const filteredProducts = allGlass.filter((item) => {
        ...
        <div className='w-100' style={{ backgroundColor: "#e9e9e9ff" }}>
          <div className='container d-flex justify-content-between pt-3 pb-3 '>
            ...
            <div>
              ...
              /
              <span className='ms-3'>
                <Link className='text-decoration-none text-hover-darkblue' to={'/glass'}>Glass</Link>
              </span>
            </div>
          </div>
        </div>
          ...
          <div className="row mt-3">
            ...
            <div className="col-12 col-lg-9 mb-2">
              <div className="d-flex flex-wrap">
                ...
                {showAll && categories.slice(2).map((cat, index) => (
                  <Button
                    key={index + 2}
                    variant={selectedCategory === cat ? "primary" : "light"}
                    className={`custom-category-btn mt-2 me-2 ${selectedCategory === cat ? "text-white fw-bold" : ""}`}
