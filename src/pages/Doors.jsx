...
import { getDoorProductsApi, getAllProductsApi } from '../services/allAPI'
...
function Doors() {
  ...
  const getAllDoors = async () => {
    try {
      const result = await getDoorProductsApi()
      if (result.status === 200) {
        setAllDoors(result.data)
      }
    } catch (err) {
      console.log(err)
    }
  ...
  const getRandomProducts = async () => {
    ...
    if (result.status === 200) {
      const nonDoors = result.data.filter(item => item.categoryGroup !== "Doors")
      const shuffled = [...nonDoors].sort(() => 0.5 - Math.random())
      setRandomProducts(shuffled.slice(0, 6)) // show 6 random products
    }
  }

  // Categories for Doors
  const categories = [
    "All",
    "Doors",
    "Door Fittings",
  ...
  // Brands (example list – replace with actual door brands if available)
  const brands = ["Europa", "Quba", "Boss", "Loctec"]

  // Filtering logic
  const filteredProducts = allDoors.filter((item) => {
        ...
        <div className='w-100' style={{ backgroundColor: "#e9e9e9ff" }}>
          <div className='container d-flex justify-content-between pt-3 pb-3 '>
            ...
            <div>
              ...
              /
              <span className='ms-3'>
                <Link className='text-decoration-none text-hover-darkblue' to={'/doors'}>Doors</Link>
              </span>
            </div>
          </div>
        </div>
        ...
        <div className='container mt-5'>
          <div className='row g-3'>
            {filteredProducts.length > 0 ? (
              filteredProducts.map((door, idx) => (
                <div 
                  key={idx} 
                  className='col-12 col-sm-6 col-md-4 col-lg-3 mt-4'
                >
                  <ProductCard displayData={door} />
                </div>
              ))
            ) : (
