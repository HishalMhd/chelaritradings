...
import { getHardwareProductsApi, getAllProductsApi } from '../services/allAPI'
...
function Hardware() {
  ...
  // Fetch only hardware products
  const getAllHardware = async () => {
    const result = await getHardwareProductsApi()
    if (result.status === 200) {
      setAllHardware(result.data)
    }
  }
  ...
  const getRandomProducts = async () => {
    ...
    if (result.status === 200) {
      const nonHardware = result.data.filter(item => item.categoryGroup !== "Hardware")
      const shuffled = [...nonHardware].sort(() => 0.5 - Math.random())
  ...
  // Filtering logic
  const filteredProducts = allHardware.filter((item) => {
      ...
      <div>
        ...
        <div className='w-100' style={{ backgroundColor: "#e9e9e9ff" }}>
          <div className='container d-flex justify-content-between pt-3 pb-3 '>
            <div className='fs-5 '>Hardware</div>
            <div>
              <span className='me-3 '>
              ...
              /
              <span className='ms-3'>
                <Link className='text-decoration-none text-hover-darkblue' to={'/hardware'}>Hardware</Link>
              </span>
            </div>
          </div>
        </div>
        ...
        <div className='container mt-5'>
          <div className='row g-3'>
            {filteredProducts.length > 0 ? (
              filteredProducts.map((hardware, idx) => (
                <div 
                  key={idx} 
                  className='col-12 col-sm-6 col-md-4 col-lg-3 mt-4'
                >
                  <ProductCard displayData={hardware} />
                </div>
              ))
            ) : (
